      ...
      <div className="p-3.5 sm:p-4 rounded-md bg-zinc-900 border border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-[11px] sm:text-xs text-zinc-400 mt-0.5">
            Variable-Length Path Query (<code className="text-zinc-300 font-mono text-[10px] sm:text-xs">Product &minus;[:REQUIRES|DEPENDS_ON*1..8]&rarr; Component</code>)
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
            <div className="p-3 sm:p-3.5 rounded-md bg-zinc-900 border border-zinc-800">
              ...
              <p className="text-[10px] sm:text-[11px] text-zinc-500 mt-0.5">Sub-assembly &amp; component nodes</p>
            </div>
            ...
            <div className="p-3 sm:p-3.5 rounded-md bg-zinc-900 border border-zinc-800">
              ...
              <p className="text-[10px] sm:text-[11px] text-zinc-500 mt-0.5 font-sans">Recursive path depth</p>
            </div>
            ...
            <div className="p-3 sm:p-3.5 rounded-md bg-zinc-900 border border-zinc-800">
              ...
              <p className="text-[10px] sm:text-[11px] text-zinc-500 mt-0.5">Highest supplier risk along path</p>
            </div>
          </div>
          ...
          <div className="rounded-md bg-zinc-900 border border-zinc-800 p-3.5 sm:p-4 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-zinc-800">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-300 truncate">
              ...
              <table className="w-full text-left text-xs font-mono min-w-[580px]">
                <thead>
                  <tr className="border-b border-zinc-800 text-[10px] uppercase tracking-wider text-zinc-400">
                    <th className="py-2 px-2 w-16">Level</th>
                ...
                <tbody className="divide-y divide-zinc-800/60">
                  {bomData.components.map((comp: any) => {
                    const depthIndent = (comp.depth - 1) * 12;
                      ...
                      <tr key={comp.id} className="hover:bg-zinc-800/40 transition-colors">
                        ...
                        <td className="py-2 px-2">
                          <div style={{ paddingLeft: `${depthIndent}px` }} className="flex items-center gap-1.5">
                            {comp.depth > 1 && <span className="text-zinc-600 font-sans text-xs">&rdsh;</span>}
                            <div>
                              <span className="font-sans font-medium text-zinc-200">{comp.name}</span>
                              <span className="text-zinc-500 ml-2 text-[10px] font-mono">{comp.sku}</span>
                            </div>
                          </div>
                        </td>
